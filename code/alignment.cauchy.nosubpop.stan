...
model {
  ...
  eta_pop ~ cauchy(0,2.5);                    #Note that this distribution may be changed if baselines are expected to be very high/low
  ...
  for(Observation in 1:NumObservations) {
    eta_observation[Observation] ~ normal(eta_pop[MarkerType[Observation]],StdDev);
    eta_ab_observation[Observation] ~ normal(eta_ab_pop[MarkerType[Observation]],StdDev);
  }
